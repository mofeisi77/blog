...
function init(baseUrl) {
  ...
  scriptMd5.src = `${baseUrl}static/js/md5.js`;
  document.head.appendChild(scriptMd5);

  scriptMd5.onload = function () {
    console.log("md5.js loaded")
    // step1. sythx highlighting
    syntaxHighlight(baseUrl);
    // step2. lazyload
    initLazyLoad(baseUrl);
  }
}


function initLazyLoad(baseUrl) {
  var script = document.createElement("script");
  script.src = `${baseUrl}static/js/animation.js`;
...
function syntaxHighlight(baseUrl) {
  var script = document.createElement("script");
  script.src = `${baseUrl}static/js/hljs.js`;
  document.head.appendChild(script);

  var styleLight = document.createElement("link");
  styleLight.rel = "stylesheet";
  styleLight.href = `${baseUrl}static/css/stackoverflow-light.min.css`;

  var styleDark = document.createElement("link");
  styleDark.rel = "stylesheet";
  styleDark.href = `${baseUrl}static/css/stackoverflow-dark.min.css`;
